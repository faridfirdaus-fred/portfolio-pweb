        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    ...
                    <a href="testimonial-form.php" class="btn btn-primary">
                                ...
                                <tbody>
                                    ...
                                    if ($result->num_rows > 0) {
                                        while($row = $result->fetch_assoc()) {
                                            echo '<tr>
                                                    <td>' . $row['id'] . '</td>
                                                    <td><img src="../uploads/' . $row['image'] . '" class="testimonial-image" alt="' . $row['name'] . '"></td>
                                                    <td>' . $row['name'] . '</td>
                                                    <td>' . $row['position'] . '</td>
                                                    <td>' . (strlen($row['content']) > 50 ? substr($row['content'], 0, 50) . '...' : $row['content']) . '</td>
                                                    <td>' . date('M d, Y', strtotime($row['created_at'])) . '</td>
                                                    <td>
                                                        <a href="testimonial-form.php?id=' . $row['id'] . '" class="btn btn-sm btn-info me-2">
                                                            <i class="fas fa-edit"></i>
                                                        </a>
                                                        <a href="testimonials.php?delete=' . $row['id'] . '" class="btn btn-sm btn-danger" onclick="return confirm(\'Are you sure you want to delete this testimonial?\')">
